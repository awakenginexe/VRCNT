...
Function ${FUNCTION_NAME}
  ...
  FileWrite $0 "$$legacyData = Join-Path $$env:LOCALAPPDATA 'VRCNT-NextData'$\r$\n"
  FileWrite $0 "if ((Test-Path -LiteralPath $$legacyData) -and -not (Test-Path -LiteralPath $$data)) { Move-Item -LiteralPath $$legacyData -Destination $$data }$\r$\n"
  FileWrite $0 "if ((Test-Path -LiteralPath $$legacyData) -and -not (Test-Path -LiteralPath $$data)) { $$data = $$legacyData }$\r$\n"
